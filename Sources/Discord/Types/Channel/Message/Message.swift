...
public struct Message: DiscordGatewayType {
...
//        return !(author.id == handler.state.me.id) && !(author.isBot ?? false)
//    }
//
//    public var channel: Channel {
//        return self.handler.state.channels[channelId]!
//    }
//
...
//
//        return self.handler.client.execute(.ChannelMessagesModify(self.channelId, self), body).map { $0 as Message }
//    }
//
//    public func delete() -> EventLoopFuture<Message> {
//        return self.handler.client.execute(.ChannelMessagesDelete(self.channelId, self)).map { $0 as Message }
//    }
//
//    func add(reaction: Emoji) {
//        self.handler.client.execute(.ChannelMessagesReactionsCreate(self.channelId, self, reaction.description))
//    }
//
//    func remove(reaction: Emoji, user: User? = nil) {
//        let user = user != nil ? "\(user!.id)" : "@me"
//
//        self.handler.client.execute(.ChannelMessagesReactionsDelete(self.channelId, self, reaction.description, user))
//    }
//
